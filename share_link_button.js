define(['jquery', 'qlik', 'angular', './propertiesPanel', 'text!./button.html', 'css!./share_link_button.css','text!./main.css'], function($, qlik, angular, propertiesPanel, button) {
    return {
        ...
        controller: ['$scope', '$element', function($scope, $element) {
                    ...
                    app.createCube({
                        ...
                        if (replyValue && replyValue != '-') {
                            ...
                            if (fieldSelections.length > 0) {
                                ...
                                createselectionURLText(fieldSelections, tagSeparator, valueSeparator, true).then(function(selectionURL) {
                                    if (selectionURL.tooManySelectionsPossible) {
                                        //console.log("Possible 'x of y values' returned. Need to double check. These dimensions are suspected: "+selectionURL.suspectedFields);
                                        //If tooManySelections is possible, then create a new hypercube with the number of selections of the suspected fields
                                        let measuresDef = [];
                                        selectionURL.suspectedFields.forEach(function(field) {
                                        ...
                                        app.createCube({
                                            ...
                                            qInitialDataFetch: [{
                                                ...
                                                qLeft: 0,
                                                qHeight: 1,
                                                qWidth: selectionURL.suspectedFields.length
                                            }]
                                        }, function(reply) {
                                            let tooManySelectionsMade = false;
                                            // Disable check too Many selectionsmode
                                            ...
                                            if (tooManySelectionsMade) {
                                                ...
                                                let selectionURL = createselectionURLText(fieldSelections, tagSeparator, valueSeparator, false);
                                                resolve([$element, config, layout, selectionURL.selectionURLText, layout.props.emailRecipients, layout.props.emailTopic, layout.props.emailBody]);
                                            }
                                        }); //end of tooManySelections hypercube
                                    } //end of tooManySelections possibility
                                    else {
                                        //If there's no possibility of too many selections, activate the button with the selections part added to the selectionURL
                                        resolve(selectionURL.selectionURLText);
                                    }
                                });
                            } //end of if split selected fields is zero
                            else {
                                //If the array of split selected fields is zero, activate the button with no selections added to the selectionURL
                                resolve(selectionURL.selectionURLText);
                            }
                        } //end of if App Integration API's reply is empty
                        else {
                            //If the app's reply is empty, activate the button with no selections added to the selectionURL
                            resolve(selectionURL.selectionURLText);
                        }
            ...
            function createselectionURLText(fieldSelections, tagSeparator, valueSeparator, checkForTooManySelections) {
                return new Promise((resolve, reject) => {
                    let returnObject = {
                        selectionURLText: "",
                    ...
                    fieldSelections.forEach(function(item) {
                        ...
                        app.model.enigmaModel.evaluate(qFormula).then(function(result) {
                            var urlArr = document.location.href.split('/');
                            var selectionURL = urlArr.splice(0, urlArr.indexOf('sheet') + 2).join('/') + '/select/' + encodeURI(result) + '%5D';
                            returnObject.selectionURLText = url.replace('/select/-%5D','');    
                        });
